        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <div class="mb-3">
                            <label class="form-label" id="labelPatrimonioCreate">Patrimônio <span class="text-danger">*</span></label>
                            <input type="text" name="patrimonio" id="inputPatrimonioCreate" class="form-control @error('patrimonio') is-invalid @enderror" 
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            <label class="form-label" id="labelNumeroSerieCreate">Número de Série <span class="text-danger">*</span></label>
                            <input type="text" name="numero_serie" id="inputNumeroSerieCreate" class="form-control @error('numero_serie') is-invalid @enderror" 
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select name="tipo" id="tipoEquipamentoCreate" class="form-select @error('tipo') is-invalid @enderror" required>
                                ...
                                <option value="nobreak" {{ old('tipo') == 'nobreak' ? 'selected' : '' }}>Nobreak</option>
                                <option value="estabilizador" {{ old('tipo') == 'estabilizador' ? 'selected' : '' }}>Estabilizador</option>
...
document.addEventListener('DOMContentLoaded', function() {
    const tipoEquipamentoCreate = document.getElementById('tipoEquipamentoCreate');
    const inputPatrimonioCreate = document.getElementById('inputPatrimonioCreate');
    const labelPatrimonioCreate = document.getElementById('labelPatrimonioCreate');
    const inputNumeroSerieCreate = document.getElementById('inputNumeroSerieCreate');
    const labelNumeroSerieCreate = document.getElementById('labelNumeroSerieCreate');
    
    if (tipoEquipamentoCreate) {
        function atualizarCampos() {
            const tipo = tipoEquipamentoCreate.value;
            const semPatrimonio = ['teclado', 'mouse'];
            
            // Patrimônio: opcional apenas para teclado e mouse
            if (semPatrimonio.includes(tipo)) {
                inputPatrimonioCreate.removeAttribute('required');
                labelPatrimonioCreate.innerHTML = 'Patrimônio (opcional)';
                inputPatrimonioCreate.placeholder = 'Opcional para este tipo';
            } else {
                inputPatrimonioCreate.setAttribute('required', 'required');
                labelPatrimonioCreate.innerHTML = 'Patrimônio <span class="text-danger">*</span>';
                inputPatrimonioCreate.placeholder = '';
            }
            
            // Número de série: obrigatório apenas para desktop
            if (tipo === 'desktop') {
                inputNumeroSerieCreate.setAttribute('required', 'required');
                labelNumeroSerieCreate.innerHTML = 'Número de Série <span class="text-danger">*</span>';
            } else {
                inputNumeroSerieCreate.removeAttribute('required');
                labelNumeroSerieCreate.innerHTML = 'Número de Série (opcional)';
            }
        }
        
        // Executa ao mudar o tipo
        tipoEquipamentoCreate.addEventListener('change', atualizarCampos);
        
        // Executa ao carregar se já houver valor selecionado
        if (tipoEquipamentoCreate.value) {
            atualizarCampos();
        }
    }
});
